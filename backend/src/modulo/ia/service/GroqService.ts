...
export class GroqService {
    // 1. Surto respiratório ou por CID específico
    async gerarRelatorioSurto(unidadeSaudeId?: string, cidPrefix = 'J') {  // 'J' para respiratórias
        const query = supabaseServiceClient
            .from('consulta')
            .select('data_consulta, cid10, observacoes')
            .eq('ativo', true)
            .gte('data_consulta', new Date(Date.now() - 400 * 24 * 60 * 60 * 1000).toISOString()); // ~13 meses para sazonalidade
    ...
    // 2. Paciente recorrente (mais rico com triagem e prescrição)
    ...
    // 3. Análise de triagens por unidade (sobrecarga)
